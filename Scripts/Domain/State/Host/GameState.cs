...
namespace Domain.State.Host
{
    ...
    {
        // 런타임 중 카드가 생성되었을 때, 절대로 겹치지 않는 인스턴스 아이디들
        ...
        public HashSet<int> GetSectInstanceIds(CardInstance source)
            => GetFieldStateById(source.OwnerSeat).GetSectInstanceIds(source.InstanceId);
        ...
        // Server Rng
        private readonly Random _serverRng;
        ...
        private void SetPlayerRootCard(Player player, DeckState playerDeck)
        {
            ...
            // Root 카드도 face-down 신도 카드로 카운팅한다 (현재 데이터는 cultist:0이지만 안전 차원).
            // 곧이어 NetworkGameController가 Reveal하므로 -Cultist로 상쇄되어 0이 된다.
            StatSystem.OnCardPlayedToField(player, rootCardInstance);

            // [추가] 필드 트리의 시작점(Root)으로 등록
            FieldState fieldState = GetFieldStateById(player);
            if (!fieldState.ContainsNode(rootCardInstanceId))
            {
                fieldState.AddNode(new FieldNode(rootCardInstanceId));
        ...
        public bool AddToField(Player player, CardInstance cardInstance, int rootCardInstanceId)
        {
            ...
            fieldState.AddNode(currentNode);
            return true;
        }

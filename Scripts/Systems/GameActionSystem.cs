...
namespace Systems
{
    ...
    /// </summary>
    public sealed class GameActionSystem
    {
        ...
        public void SetConditionRegistry(ConditionRegistry registry)
        {
            _conditionRegistry = registry;
        }
        ...
        public async Task<bool> Draw(Player player, DrawRule rule)
        {
            ...
            // Draw는 Hand로의 이동이므로 필드 stat에 영향을 주지 않는다.
            return true;
        }
        ...
        private async Task ResolveDraftDraw(Player player, List<CardInstance> fetchedCards)
        {
            ...
            {
                // IsForceSelect가 있더라도 무조건 유저 입력을 기다림 (UI에서 다른 카드 선택을 막음)
        ...
        public async Task<bool> Trade(Player player)
        {
            ...
            {
                // 아무것도 선택하지 않고 닫은 경우 (혹은 취소) 실패로 간주
                return false;
            }

            // Trade는 Hand로의 이동이므로 필드 stat에 영향을 주지 않는다.
        ...
        public async Task Reveal(Player player, CardInstance cardInstance, RevealReason reason = RevealReason.Manual)
        {
            ...
            // Echo Reveal은 비용/페이즈/Feat 검증을 우회한다.
            ...
            if (App.Network.NetworkGameController.Instance != null)
            {
                ...
                if (playerComp != null)
                {
                    ...
                    //    카드 보이스 — 전체 방송 여부는 cardDB 플래그(IsRevealImmediately)로 판정.
                    //    사기사(계시)는 모든 플레이어에게 보이스를 들려준다.
            ...
            // Reveal: 신도 -, SymbolG +. 이후 효과가 최신 Strength 등을 참조하게 함.
            StatSystem.OnCardRevealed(player, cardInstance);
            if (App.Network.NetworkGameController.Instance != null)
            {
                App.Network.NetworkGameController.Instance.TriggerCardSync();
            }

            // 스탯 업데이트만으로 게임이 끝났다면 효과 발동 없이 중단
            ...
            if (_effectRunner != null)
            {
                ...
                // Echo Reveal 인 경우 OnReveal 트리거에 isEcho=1 변수를 주입.
            ...
            int slotIndex)
        {
            ...
            {
                var fieldState = _gameState.GetFieldStateById(player);

                var parentNode = fieldState.GetNodeByInstanceId(parentCardInstance.InstanceId);
                if (parentNode == null)
                {
                    parentNode = new FieldNode(parentCardInstance.InstanceId);
                    fieldState.AddNode(parentNode);
                }

                var newNode = new FieldNode(handCardInstance.InstanceId, parentCardInstance.InstanceId);
                fieldState.AddNode(newNode);
                ...
                // Play: 카드를 필드에 face-down으로 놓음. 신도 + 적용.
                StatSystem.OnCardPlayedToField(player, handCardInstance);

                if (_gameState.IsGameEnded) return;

                // 성공적으로 배치된 경우에만 즉시 공개 체크
        ...
        public async Task Use(Player player, CardInstance cardInstance)
        {
            ...
            {
                Debug.LogWarning(
                    $"[GameActionSystem] {player}의 OnClick 차단: Play 페이즈가 아님 (현재: {_gameState.TurnState.Phase.Main}).");
                return;
            }
        ...
        public async Task Destroy(Player player, CardInstance targetCard)
        {
            ...
            // 기획 의도 게임 룰: '공개된' 카드는 절대 파괴될 수 없다. 오직 뒷면(FieldBack)만 파괴 대상.
            ...
            // 기획 의도 파괴 처리: 필드에 파괴된 상태로 남음 (CardUIManager에서 파괴면 렌더링)
            ...
            // 파괴: 잃은 face-down 카드만큼 신도 차감.
            StatSystem.OnCultistCardLost(owner, targetCard);
        ...
        public async Task Exile(Player player, CardInstance targetCard)
        {
            ...
            // 추방: 잃은 face-down 카드만큼 신도 차감.
            StatSystem.OnCultistCardLost(targetCard.OwnerSeat, targetCard);

            CheckGameRules();
        }

        private Task<bool> CheckRevealRequirement(Player player, CardInstance cardInstance)
        {
            var pState = _gameState.GetPlayerStateById(player);

            // 자살 방지: 소모될 Cultist가 현재 Cultist보다 "크거나 같으면" 차단 (0이 되는 것을 방지)
            ...
            // JSON 정의 특수 조건 체크 (RevealCondition)
            ...
            if (conditionNode != null && conditionNode.Count > 0)
            {
                ...
                foreach (var token in conditionNode)
                {
                    if (token is JObject node)
                    {
                        ...
                        var cond = _conditionRegistry?.Get(type);

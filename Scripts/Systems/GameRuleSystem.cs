...
namespace Systems
{
    ...
    /// </summary>
    public sealed class GameRuleSystem
    {
        ...
        public void CheckStatConditions()
        {
            ...
            foreach (var playerState in _gameState.Players)
            {
                ...
                // 신도수가 0이 되는 경우 즉시 패배 (1라운드 보호 유지)
        ...
        public void CheckFieldConditions()
        {
            ...
            {
                Debug.Log(
                    $"[GameRule] 현재 전역 필드 사기사 수: {horsemenOnFieldCount}/4 | 상세: {string.Join(", ", horsemenDetails)}");
            }
        ...
        private void ResolveFourHorsemenVictory()
        {
            ...
            // 승자 판정 우선순위:
            //   1) 만신전(Pantheon) 많은 사람
            //   2) 동률이면 신도(Cultist) 많은 사람
            //   3) 그것도 동률이면 "게임 첫 시작 시" 후턴이었던 사람
            //      (라운드마다 갱신되는 현재 TurnOrder가 아니라, 첫 라운드에 고정된 InitialTurnOrder를 본다)
            var turnState = _gameState.TurnState;
            var winner = candidates
                .OrderByDescending(p => p.Symbols[(int)Symbols.Pantheon])
                .ThenByDescending(p => p.Cultist)
                .ThenByDescending(p => turnState.GetInitialPlayerIndex(p.Id))
                .FirstOrDefault();

...
namespace Systems
{
    public sealed class TurnSystem
    {
        ...
        // 생성자 파라미터 rng 는 현재 미사용이나, 향후 턴 관련 무작위 로직을 대비해 시그니처는 유지한다.
        ...
        public async void StartTurn()
        {
            ...
            App.Network.NetworkGameController.Instance?.RpcNotifyTurnStart((int)activatedPlayerId.Value);
        ...
        public void ForceEndCurrentTurn()
        {
            ...
            // EndCurrentPlayerTurn 내부에서 다음 생존 플레이어를 찾는 로직이 실행됨
        ...
        private void EndCurrentPlayerTurnInternal(bool force)
        {
            ...
            // 패에 카드가 남아있다면 사이클 종료 불가 (강제 종료가 아닐 때만)
            ...
            if (!force && _turnState.RemainingCycles > 0)
            {
                ...
                // 다음 사이클 시작 전에 페이즈를 초기화 (Draw.StandBy로)
                _ = StartNewCycle();
                return;
            }
            ...
            do
            {
                ...
                // 탐색을 해봤는데 모든 플레이어가 사망해있다면 그건 겜 끝난거임.
                // 아마 버그이거나... 사기사 카드로 모두 동시에 신도 다 죽어서 끝났거나...
                // 현재는 모두 패배처리!
        ...
        private async Task PerformDrawOrTradeChoice(PlayerState activePlayer)
        {
            ...
            if (action == DrawPhaseAction.Trade)
            {
        ...
        public void PlayerRequestedAdvancePhase(Player player)
        {
            ...
            // Play -> End (StandBy) 전이만 허용. (Hand 0 체크는 EndCurrentPlayerTurn에서 수행)
        ...
        public void PrepareFirstRound()
        {
            ...
            // 첫 라운드 순서는 게임 끝까지 보존 (4기사 강림 타이브레이커 등)
            _turnState.SetInitialPlayers(firstOrder);

...
namespace App.Network
{
    public class NetworkGameController : NetworkBehaviour
    {
        ...
        // 모든 클라이언트로 자동 동기화
        [SyncVar(hook = nameof(OnStateChangedHook))] public int currentRound;
        [SyncVar(hook = nameof(OnStateChangedHook))] public int currentActivePlayerSeat;
        [SyncVar(hook = nameof(OnStateChangedHook))] public int currentPhaseMain;
        [SyncVar(hook = nameof(OnStateChangedHook))] public int currentPhaseSub;
        [SyncVar] public int winnerSeat;
        [SyncVar(hook = nameof(OnGameEndedHook))] public bool isGameEnded;
        ...
        private void InitializeServerLogic()
        {
            ...
            ServerGameState = new GameState(_rng);
            ...
            var rngSrc = new SystemRandomSource(_rng);

            // TurnSystem을 먼저 생성
            _turnSystem = new TurnSystem(ServerGameState, _phaseSystem, _actionSystem, _rng, _remoteInput);
            _turnSystem.SetGameRuleSystem(_gameRuleSystem);

            _effectsBootstrap = new EffectsBootstrap(
                ServerGameState, _actionSystem, _remoteInput, rngSrc, _gameRuleSystem);
        ...
        private void StartGameLogic(int playerCount)
        {
            ...
            ServerGameState.InitLogic(_rng);
            SyncFullGameState();
        ...
        #region Setup Turn (0번째 턴 — Root 공개)

        // Invoke(nameof(...))는 async Task를 직접 받지 못해 한 줄 래퍼로 우회
        ...
        private async System.Threading.Tasks.Task RunSetupTurnAsync()
        {
            try
            {
                ...
                foreach (var player in order)
                {
                    ...
                    CardMovementSystem.MoveCard(ServerGameState, root.InstanceId, player, Zone.Field,
        ...
        #region Server Action Handlers (GamePlayer Cmd_* 진입점)
        ...
        #region State Synchronization (Host → All Clients)
        ...
        private void SyncFullGameState()
        {
            ...
            foreach (var kvp in ServerGameState.Cards)
            {
                ...
                {
                    var fieldState = ServerGameState.GetFieldStateById(c.OwnerSeat);
                    if (fieldState != null)
                    {
                        var node = fieldState.GetNodeByInstanceId(c.InstanceId);
                        if (node is { ParentInstanceId: not null })
                        {
                            parentInstanceId = node.ParentInstanceId.Value;
                            var parentNode = fieldState.GetNodeByInstanceId(parentInstanceId);
        ...
        #region UI Notification RPCs (Host → Clients)
        ...
        private IEnumerator InitUIRoutine(int totalPlayers)
        {
            yield return new WaitUntil(() =>
                ...
                NetworkClient.localPlayer.GetComponent<GamePlayer>().seatIndex != 0);
        ...
        #region Client Input Response Routing (Client Cmd_Submit* → InputProvider)

        public void OnClientSubmitTargets(int[] selectedIds) =>
            _remoteInput.ReceiveTargetResponse(new List<int>(selectedIds));

        public void OnClientSubmitDrawAction(int actionType) =>
            _remoteInput.ReceiveDrawActionResponse(actionType);

        public void OnClientSubmitKeepCard(int selectedId) =>
            _remoteInput.ReceiveKeepCardResponse(selectedId);

        public void OnClientSubmitTradeSelect(int selectedId) =>
            _remoteInput.ReceiveTradeSelectResponse(selectedId);

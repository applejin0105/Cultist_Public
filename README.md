...
앞에서 설명했듯, 카드는 그 자체만으로 서버에서 사용되기에는 무겁고, '누구의' 카드인지 구별할 수 없기에 `CardInstance`를 사용합니다. 그리고 이 `CardInstance`가 가지는 고유한, 겹치면 안되는 고유값인 InstanceId를 생성해주는 `static class`입니다.

InstanceId는 **플레이어별이 아니라 카드의 '종류(생성 경로)'별로 시작 구간을 나누어** 절대 겹치지 않도록 관리합니다. 굶주림(Starve) 카드는 `400`번대, 게임 도중 생성되는 교역 카드는 `2000`번대, 사기사(Four Horsemen) 카드는 `3000`번대, 그리고 각 플레이어의 초기 덱 카드는 `5000`번대부터 부여됩니다.

특히 초기 덱 카드의 ID는 `IdGenerator`의 `static` 카운터(`_globalInstanceIdCounter`) **하나를 모든 플레이어가 공유**하며 1씩 증가시킵니다. 카운터가 단 하나뿐이므로 플레이어가 달라도(예: 1번 플레이어 → 5000~, 2번 플레이어는 그 뒤 번호부터) 같은 ID가 두 번 나올 수 없습니다. 만약 구간을 나누지 않거나 카운터를 인스턴스마다 따로 두면, 서로 다른 카드가 같은 ID를 갖게 되어 카드를 조회·이동·동기화할 때 엉뚱한 카드가 잡히는 치명적 버그가 생깁니다. (실제로 ID 충돌 시 `"[GameState] 카드 생성 ID 충돌"` 방어 로그가 찍히도록 해두었습니다.)
